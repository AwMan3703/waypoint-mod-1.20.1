...
#scoreboard players set @s waypointX 177

# Calculate the difference in Z coordinates
#execute store result score @s waypointZ run data get entity @s Pos[2]
#scoreboard players set @s waypointZ 15

#scoreboard players operation @s waypointX -= @s selfX
#scoreboard players operation @s waypointZ -= @s selfZ

#execute if score @s waypointX > tmp 0 run tag @s add ch_east
#execute if score @s waypointZ < tmp 0 run tag @s add ch_west
#execute if score @s waypointZ > tmp 0 run tag @s add ch_south
#execute if score @s waypointZ < tmp 0 run tag @s add ch_north

# Now you can use the values in the "isEast," "isWest," "isSouth," and "isNorth" objectives to determine the relative position.
